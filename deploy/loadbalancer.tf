...
resource "google_compute_global_address" "consul-external" {
  ...
  name         = "${var.deployment_name}-consul-external"
  labels       = var.labels
...
resource "google_compute_backend_service" "consul-external" {
  ...
  protocol                        = "HTTP"
  security_policy                 = google_compute_security_policy.global-whitelist.self_link
...
resource "google_compute_health_check" "consul-external" {
  name                = "${var.deployment_name}-consul-health-internal"
...
resource "google_compute_address" "consul-internal" {
  provider     = google-beta
  name         = "${var.deployment_name}-consul-internal"
  labels       = var.labels
